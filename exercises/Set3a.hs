...
capitalize s = unwords (map (\(first:rest) -> toUpper first:rest) (words s))
...
powers :: Int -> Int -> [Int]
powers k max = takeWhile (\x -> x <= max) [k^i | i <- [0..max-1]]
-- powers k max = takeWhile (\x -> x^max <= max) [k^i | i <- [0..max]]
...
joinToLength n strings = [first ++ second | first <- strings, second <- strings, length (first ++ second) == n]
...
sumRights xs = sum (map (\x -> either (\y -> 0) id x) xs)
...
multiCompose [] = id
multiCompose fs = head fs . multiCompose (tail fs)
...
interpreter' commands x y
    | head commands == "up" = interpreter' (tail commands) x (y+1)
    | head commands == "down" = interpreter' (tail commands) x (y-1)
    | head commands == "left" = interpreter' (tail commands) (x-1) y
    | head commands == "right" = interpreter' (tail commands) (x+1) y
    | head commands == "printX" = show x : (interpreter' (tail commands) x y)
    | head commands == "printY" = show y : (interpreter' (tail commands) x y)
    | otherwise = interpreter' (tail commands) x y
