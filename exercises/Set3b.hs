...
sums' i j
    | j > i = []
    | otherwise = j * (j+1) `div` 2 : sums' i (j+1)
...
sorted' [] _ = True
sorted' (x:xs) prev = if x >= prev then sorted' xs x else False
...
sumsOf' [] _ = []
sumsOf' (x:xs) sum = x + sum : sumsOf' xs (x+sum)
...
maybeMap _ [] = []
maybeMap f (x:xs) = if isNothing (f x) then maybeMap f xs else fromJust (f x) : maybeMap f xs
